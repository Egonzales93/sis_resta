                ...
                success: function (data) {
                    response($.map(data, function (item) {
                        ...
                        return {
                            ...
                            label: tipo_cli+': '+item.ci+''+item.nit+' | '+item.nombre,
                            value: tipo_cli+': '+item.ci+''+item.nit+' | '+item.nombre
                        }
                    }))
                }
            })
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if(data.id_tipo_pedido == 1){
                ...
                $('#cliente_id').val(1);
                $('#buscar_cliente').val('CI: 00000000 | PUBLICO EN GENERAL');
                ...
                if(data.id_tipo_pedido == 2){
                    ...
                    $('#buscar_cliente').val(tipo+': '+data.ci_cliente+''+data.nit_cliente+' | '+data.nombre_cliente);
...
var pedido = {
    ...
    refrescar: function()
    {
        ...
        this.detalle.iva      = (this.detalle.total * 0.18).toFixed(2); // 18 % El IVA y damos formato a 2 deciamles
        ...
        if($('#codimpcomandamesa').val() == 1){
            ...
            if(($('#codtipoped').val() == 1) || $('#codtipoped').val() == 2){
                ...
                for(y=0; y < cont.length; y++){
                    ...
                    if($('#print_com').val() == 1 ){
                        console.log('prin activo');
                        window.open('http://'+$('#pc_ip').val()+'/imprimir/comanda.php?data='+JSON.stringify(nuevopedido)+'','_blank');
                    }else{
                        console.log('prin inactivo');
                        // var Urlpdf = $("#url").val()+'informe/venta_all_imp_/'+url; 
                        window.open(''+$("#url").val()+'venta/impresion_comanda/?data='+JSON.stringify(nuevopedido)+'', "_blank", "top=500,left=500,width=400,height=400");
                        // window.open(''+$("#url").val()+'/venta/impresion_comanda/?data='+JSON.stringify(nuevopedido)+'','_blank');
                    }
