...
class Venta_Model extends Model
{
    ...
    */
    ...
    public function cliente_crud_create($data)
    {
        try 
        {
            $consulta = "call usp_restRegCliente( :flag, @a, :tipo_cliente, :ci, :nit, :nombres, :razon_social, :telefono, :fecha_nac, :correo, :direccion, :referencia);";
                $arrayParam =  array(
                    ':flag' => 1,
    ...
    public function cliente_crud_update($data)
    {
        try 
        {
            $consulta = "call usp_restRegCliente( :flag, :id_cliente, :tipo_cliente, :ci, :nit, :nombres, :razon_social, :telefono, :fecha_nac, :correo, :direccion, :referencia);";
                $arrayParam =  array(
                    ':flag' => 2,
    ...
    public function buscar_cliente($data)
    {
        ...
        {        
            ...
            if($data['tipo_cliente'] == 1 OR $data['tipo_cliente'] == 2){
                $stm = $this->db->prepare("SELECT * FROM v_clientes WHERE tipo_cliente = ".$data['tipo_cliente']." AND estado <> 'i' AND (ci LIKE '%".$data['cadena']."%' OR nit LIKE '%".$data['cadena']."%' OR nombre LIKE '%".$data['cadena']."%') ORDER BY ci LIMIT 5");
            } else {
                $stm = $this->db->prepare("SELECT * FROM v_clientes WHERE estado <> 'i' AND (ci LIKE '%".$data['cadena']."%' OR nit LIKE '%".$data['cadena']."%' OR nombre LIKE '%".$data['cadena']."%') ORDER BY ci LIMIT 5");
            }
            $stm->execute();
    ...
    public function buscar_cliente_telefono($data)
    {
        ...
        {        
            /*nota: para kerlyn quitar el tipo_cliente de la consulta*/
            $stm = $this->db->prepare("SELECT * FROM v_clientes WHERE estado <> 'i' AND (ci LIKE '%".$data['cadena']."%' OR nit LIKE '%".$data['cadena']."%' OR nombre LIKE '%".$data['cadena']."%' OR telefono LIKE '%".$data['cadena']."%') ORDER BY ci LIMIT 5");
            $stm->execute();
            return $stm->fetchAll(PDO::FETCH_OBJ); 
        }
        catch(Exception $e)
    ...
    */
    ...
    public function RegistrarVenta($data)
    {
        try
        {
            ...
            $consulta = "call usp_restEmitirVenta(:flag, :dividir_cuenta, :id_pedido, :tipo_pedido, :tipo_entrega, :id_cliente, :id_tipo_doc, :id_tipo_pago, :id_usu, :id_apc, :pago_efe_none, :pago_tar, :descuento_tipo, :descuento_personal, :descuento_monto, :descuento_motivo, :comision_tarjeta, :comision_delivery, :iva, :total, :codigo_operacion, :fecha_venta);";
            $arrayParam = array(
                ':flag' => 1,
    ...
    public function contadorImpuestosSinEnviar()
    {
        try
        {   
            ...
            $stm = $this->db->prepare("SELECT COUNT(v.id_ven) AS total FROM v_ventas_con AS v INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc INNER JOIN tm_tipo_doc AS d ON v.id_tdoc = d.id_tipo_doc WHERE v.ser_doc = d.serie AND v.id_tdoc <> 3 AND v.estado = 'a' AND (v.enviado_impuestos = '' OR v.enviado_impuestos = '0' OR v.enviado_impuestos IS NULL)");
            $stm->execute();        
            $c = $stm->fetch(PDO::FETCH_OBJ);
