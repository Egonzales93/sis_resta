...
class Inventario_Model extends Model
{
    ...
    public function Responsable()
    {
        try
        {      
            return $this->db->selectAll('SELECT * FROM tm_usuario WHERE id_usu <> 1 GROUP BY ci');
        }
        catch(Exception $e)
        {
            die($e->getMessage());
    ...
    public function ajuste_insumo_buscar($data)
    {
        ...
        {        
            $cadena = $data['cadena'];
            // $stm = $this->db->prepare("SELECT * FROM v_insprod WHERE (ins_cod LIKE '%$cadena%' OR ins_nom LIKE '%$cadena%') AND est_b = 'a' AND est_c = 'a'  ORDER BY ins_nom LIMIT 5 AND crt_stock = '1'");
            $stm = $this->db->prepare("SELECT * FROM v_insprod WHERE (ins_cod LIKE '%$cadena%' OR ins_nom LIKE '%$cadena%') AND est_b = 'a' AND est_c = 'a' AND (crt_stock = '1' OR ins_rec= '1' ) ORDER BY ins_nom LIMIT 5");
            $stm->execute();
            $c = $stm->fetchAll(PDO::FETCH_OBJ);
            return $c;
        }
