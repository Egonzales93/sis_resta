    ...
    public function getCabecera($cod_ven)
    {
        ...
        {      
            ...
            $c->{'Detalle'} = $this->db->query("SELECT v_productos.pro_cod AS codigo_producto, 
                ...
                IF(v_productos.pro_imp='1',ROUND((tm_detalle_venta.precio/(1 + 0.18)),6),tm_detalle_venta.precio) AS valor_unitario,
    ...
    public function registrar_resumen($cabecera , $detalle)
    {
        ...
        try {
            ...
            foreach ($detalle as $clave => $valor)
            {
                ...
                if ($result) {
                    $this->db->query("UPDATE tm_venta SET enviado_impuestos=1 WHERE id_venta='".$valor['id_venta']."'");
                    $status = true;
                }else{
                    $status = false;
