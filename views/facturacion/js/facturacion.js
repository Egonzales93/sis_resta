...
var lisTab1 = function(){
        ...
        success: function(item){
            if (item.data.length != 0) {
                $.each(item.data, function(i, campo) {
                    if(campo.estado == 'a' && campo.enviado_impuestos == '1'){
                        total_env++;
    ...
    .DataTable({
        ...
                return '<div>'+data.desc_td+'<br>'+data.ser_doc+'-'+data.nro_doc+'</div>';
            ...
            {"data":null,"render": function ( data, type, row ) {
                return '<div class="mayus">'+data.Cliente.ci+''+data.Cliente.nit+'<br>'+data.Cliente.nombre+'</div>';
            }},
            {"data":null,"render": function ( data, type, row) {
        ...
                    return '<span class="label label-primary">ENVIADO A IMPUESTOS</span></a>';
            ...
            {"data":null,"render": function ( data, type, row ) {
                if(data.hash_cpe != ''){
                    return '<center><a href="'+$("#entorno").val()+'/'+data.name_file_impuestos+'.XML" target="_blank"><img src="public/images/xml_cpe.svg" style="max-width: 30px;"/></a></center>';
                } else{
            ...
            {"data":null,"render": function ( data, type, row ) {
                if(data.hash_cdr != ''){
                    return '<center><a href="'+$("#entorno").val()+'/R-'+data.name_file_impuestos+'.XML" target="_blank"><img src="public/images/xml_cdr.svg" style="max-width: 30px;"/></a></center>';
                } else {
                    return '<center>-</center>';
                }
            }},
            {"data":null,"render": function ( data, type, row ) {
                return '<div style="position: relative; width: 30px; float: left; vertical-align: middle;"><a href="'+$("#url").val()+'informe/venta_all_imp_a4/'+data.id_ven+'" target="_blank"><img src="public/images/pdfa4.png" style="max-width: 29px;"/></a></div> <div style="position: relative;  width: 29px; float: right; vertical-align: middle;"><a href="'+$("#url").val()+'informe/venta_all_imp/'+data.id_ven+'" target="_blank"><img src="public/images/pdf.svg" style="max-width: 29px;"/></a></div>';
            }},
            {"data":null,"render": function ( data, type, row ) {
                return '<center><a href="#" onclick="send_mail('+data.id_ven+',\''+$("#entorno").val()+'/'+data.name_file_impuestos+'.XML\');"><img src="public/images/email.svg" style="max-width: 30px;"/></a></center>';
            }},
            {"data":null,"render": function ( data, type, row ) {
                if(data.enviado_impuestos == '1'){
                ...
                +'<i class="'+clase+'" style="font-height: bold; font-size: 20px;"></i></button>'
                    +'<div class="dropdown-menu dropdown-menu-right p-4" style="width: 300px; font-size: 14px">'
                        ...
                        +'<p class="m-b-2 text-left">Código Respuesta: <span class="label label-primary">'+data.code_respuesta_impuestos+'</span></p>'
                        +'<p class="m-b-0">Descripción: <span class="text-muted">'+data.descripcion_impuestos_cdr+'</span></p>'
                        +opcion
                        +opcion2
                    +'</div>'
                +'</div></div>';
...
var lisTab4 = function(){
    ...
    .DataTable({
        ...
                    return '<span class="label label-primary">ENVIADO A IMPUESTOS</span></a>';
            ...
            {"data":null,"render": function ( data, type, row ) {
                if(data.hash_cpe != null && data.enviado_impuestos == '1'){
                    return '<center><a href="'+$("#entorno").val()+'/'+data.name_file_impuestos+'.XML" target="_blank"><img src="public/images/xml_cpe.svg" style="max-width: 30px;"/></a></center>';
                } else {
                    return '<center>-</center>';
                }
            ...
            {"data":null,"render": function ( data, type, row ) {
                if(data.code_respuesta_impuestos=='0'){
                    var clase = 'ti-check text-success';
                    var opcion = '';
                }else{
                    var clase = 'ti-close text-danger';
...
var lisTab6 = function(cod){
    ...
    .DataTable({
        ...
        "columns":[
            ...
            {"data":null,"render": function ( data, type, row ) {
                return data.Cliente.ci+' - '+data.Cliente.nombre;
            }},
            {"data":null,"render": function ( data, type, row ) {
...
var invoice = function(cod_ven,doc){
    var text1 = '¿Realmente deseas enviar el documento  N°: '+doc+' a IMPUESTOS? <br>Lo enviaremos inmediatamente!..';
    Swal.fire({
        ...
        preConfirm: function() {
          return new Promise(function(resolve) {
             ...
             .done(function(response){
                if(response['enviado_impuestos'] == '1'){
                    Swal.fire({
...
var reenvio = function(cod_ven,doc){
    var text1 = 'El documento  N°: '+doc+' ya ha sido enviado anteriormente a IMPUESTO <br>Actualizaremos los datos en el sistema!..';
    ...
    //contadorImpuestosSinEnviar();
}

var ComunicacionBaja = function(tipo_doc,cod_ven,doc){
    var text1 = '¿Realmente deseas dar de baja al documento  N°: '+doc+' a IMPUESTOS? <br>Lo enviaremos inmediatamente!..';
    //var text2 = 'El documento N°: '+doc+' fué enviado correctamente a Impuestos.';
    Swal.fire({
        title: 'Debes Confirmar!',
        html: text1,
        icon: 'warning',
