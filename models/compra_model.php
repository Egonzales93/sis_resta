...
class Compra_Model extends Model
{
    ...
    public function Proveedor()
    {
        try
        {      
            return $this->db->selectAll('SELECT id_prov,nit,razon_social FROM tm_proveedor');
        }
        catch(Exception $e)
        {
            die($e->getMessage());
    ...
    public function compra_crud_create($data)
    {
        ...
        {   
            ...
            $sql = "INSERT INTO tm_compra (id_prov, id_tipo_compra, id_tipo_doc, id_usu, fecha_c, hora_c, serie_doc, num_doc, iva, total, descuento, fecha_reg) VALUES (?,?,?,?,?,?,?,?,?,?,?,?);";
            $this->db->prepare($sql)
                ->execute(
                    array(
                        $data['id_prov'],
    ...
    public function compra_proveedor_buscar($data)
    {
        try
        {        
            $stm = $this->db->prepare("SELECT id_prov,nit,razon_social FROM tm_proveedor WHERE estado <> 'i' AND (nit LIKE '%$data%' OR razon_social LIKE '%$data%') ORDER BY nit LIMIT 5");
            $stm->execute();
    ...
    public function compra_proveedor_nuevo($data)
    {
        try
        {
            $consulta = "call usp_comprasRegProveedor( :flag, @a, :nit, :razon_social, :direccion, :telefono, :email, :contacto);";
            $arrayParam =  array(
                ':flag' => 1,
                ':nit' => $data['nit'],
    ...
    public function proveedor_crud_update($data)
    {
        try 
        {   
            $consulta = "call usp_comprasRegProveedor( :flag, :id_prov, :nit, :razon_social, :direccion, :telefono, :email, :contacto);";
            $arrayParam =  array(
                ':flag' => 2,
