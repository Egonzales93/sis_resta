...
class Informe_Model extends Model
{
    ...
    public function venta_all_list()
    {
        ...
        {   
            ...
            $stm = $this->db->prepare("SELECT v.id_ven,v.id_ped,v.id_tped,v.id_tpag,v.pago_efe,v.pago_tar,v.desc_monto,v.comis_tar,v.comis_del,v.total AS stotal,v.fec_ven,v.desc_td,v.ser_doc,v.nro_doc,v.estado,CASE WHEN v.estado = 'a' THEN v.pago_efe + v.pago_tar ELSE 0 END AS total,v.id_cli,v.iva,v.id_usu,v.desc_tipo,v.desc_personal,c.desc_caja, c.id_apc FROM v_ventas_con AS v INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc WHERE (DATE(v.fec_ven) >= ? AND DATE(v.fec_ven) <= ?) AND v.id_tped like ? AND v.id_tdoc like ? AND v.id_cli like ? AND v.estado like ? GROUP BY v.id_ven");
            ...
            foreach($c as $k => $d)
            {
                $c[$k]->{'Cliente'} = $this->db->query("SELECT tipo_cliente, ci, nit, nombre FROM v_clientes WHERE id_cliente = ".$d->id_cli)
                    ->fetch(PDO::FETCH_OBJ);
            }
            foreach($c as $k => $d)
            {
    ...
    public function venta_delivery_list()
    {
        try
        {
            ...
            $stm = $this->db->prepare("SELECT v.id_ven,v.id_ped,v.id_cli,v.id_apc,v.desc_td,v.ser_doc,v.nro_doc,IFNULL((v.total),0) AS total,v.comis_del, v.fec_ven,d.tipo_entrega,d.id_repartidor,d.desc_repartidor FROM v_ventas_con AS v INNER JOIN v_pedido_delivery AS d ON v.id_ped = d.id_pedido WHERE (DATE(v.fec_ven) >= ? AND DATE(v.fec_ven) <= ?) AND d.id_repartidor LIKE ? AND d.tipo_entrega LIKE ? AND v.id_tped = 3");
    ...
    //[OBTENER LA IP DE IMPRESIÓN]
    public function obtenerImpresoras(){
       try
       {   

        $stmm = $this->db->prepare("SELECT nombre, ip 
                                    FROM `tm_impresora` WHERE estado = 'a'");
        $stmm->execute();
    ...
    public function oper_anul_list($data)
    {
        try
        {
            $ifecha = date('Y-m-d H:i:s',strtotime($data['ifecha']));
            $ffecha = date('Y-m-d H:i:s',strtotime($data['ffecha']));
            $stm = $this->db->prepare("SELECT *,(cant*precio) AS total  FROM tm_detalle_pedido WHERE (DATE(fecha_pedido) >= ? AND DATE(fecha_pedido) <= ?) AND id_usu like ? AND estado = 'z'");
            $stm->execute(array($ifecha,$ffecha,$data['id_usu']));
            $c = $stm->fetchAll(PDO::FETCH_OBJ);
            foreach($c as $k => $d)
            {
       ...
       try
       {
        ...
        //    $ed     = ($tipo_ip == '2') ? 'id_tipo_ins = "2"': 'id_tipo_ins = "1"' ;
           $stm = $this->db->prepare("SELECT id_inv,id_tipo_ope,id_ope,id_tipo_ins,id_ins,cos_uni,cant,fecha_r,estado,
               ...
               FROM tm_inventario WHERE id_ins = ? AND (date(fecha_r) >= ? AND date(fecha_r) <= ?)");
           $stm->execute(array($id_ip,$ifecha,$ffecha));
           $c = $stm->fetchAll(PDO::FETCH_OBJ);
           foreach($c as $k => $d)
           {
