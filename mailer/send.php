    ...
    public function sendEmail($correo_cliente,$documento_cliente,$datos_factura,$negocio) {
        ...
        try {
            ...
            if ($data['Cliente']['tipo_cliente'] == 1){
                $tipodoc = 'CI';
                $nrodoc = $data['Cliente']['ci'];
            }else if ($data['Cliente']['tipo_cliente'] == 2){
                $tipodoc = 'NIT';
                $nrodoc = $data['Cliente']['nit'];
            }
            $pdf->SetFont('Arial','B',10);    
            ...
            // NRO DE FACTURA
            $elec = (($data['id_tdoc'] == 1 || $data['id_tdoc'] == 2) && $data['Empresa']['impuestos'] == 1) ? 'ELECTRONICA' : '';
            $pdf->SetLineWidth(0.1); $pdf->SetFillColor(500); 
            $pdf->Rect(120, 10, 85, 30);
            $pdf->SetXY( 120, 15 ); 
            $pdf->SetFont( "Arial", "B", 12 ); 
            $pdf->Cell( 85, 4, 'N.I.T. Nro. '.utf8_decode($data['Empresa']['nit']), 0, 0, 'C');
            $pdf->SetXY( 120, 15 ); 
            ...
            $pdf->Cell(5,$textypos,"BOLIVIANOS");
            ...
            //// Arreglo de Productos
                ...
                foreach($data['Detalle'] as $row)
                {
                    if($row['codigo_afectacion'] == '10'){
                        ...
                        $total_iva_gravadas = $total_iva_gravadas + $row['total_iva'];
                        ...
                        $total_iva_exoneradas = $total_iva_exoneradas + $row['total_iva'];
                    }

                    $pdf->Cell($w[0],6,utf8_decode($row['nombre_producto']),1);
                    $pdf->Cell($w[1],6,number_format($row['cantidad']),'1',0,'C');
                    $pdf->Cell($w[2],6,"Bs/  ".number_format($row['precio_unitario'],2,".",","),'1',0,'R');
                    $pdf->Cell($w[3],6,"Bs/  ".number_format($row['cantidad']*$row['precio_unitario'],2,".",","),'1',0,'R');
            ...
            /*
            $operacion_gravada = (($data['total'] + $data['comis_tar'] + $data['comis_del'] - $data['desc_monto']) / (1 + $data['iva']));
            $iva = ($operacion_gravada * $data['iva']);
            */
            //$header = array("", "");
            $data2 = array(
                ...
                array("IVA", number_format(($total_iva_gravadas + $total_iva_exoneradas),2)),
            ...
            $pdf->Ln(5);
            $pdf->setX(10);
            $pdf->Cell(5,$textypos,utf8_decode('Designado Emisor Electrónico según Resolución de Superintendecia Nro.155-2017/.'),'C');
            $pdf->Ln(5);
            $pdf->setX(10);
            ...
            $mail->addStringAttachment($fxml, utf8_decode($data['Empresa']['nit']).'-'.$data['ser_doc'].'-'.$data['nro_doc'].'.XML');
            
            //$mail->addStringAttachment($pdfdoc, 'my-doc.pdf');
            $mail->AddStringAttachment($pdfdoc, utf8_decode($data['Empresa']['nit']).'-'.$data['ser_doc'].'-'.$data['nro_doc'].'.pdf', 'base64', 'application/pdf');
                
            ## MENSAJE A ENVIAR
            //$mail->setFrom('info@xxxx.com', $negocio);
            $negocio = EMISOR;
            $mail->setFrom(DE, $negocio);
