...
class Facturacion_Model extends Model
{
	...
	public function Datos1()
    {
        ...
        {
            if($_POST['estado'] == 1){
                $estado = "AND v.estado = 'a' AND v.enviado_impuestos = '1'";
            } else if ($_POST['estado'] == 2){
                $estado = "AND v.estado = 'a' AND (v.enviado_impuestos = '' OR v.enviado_impuestos = '0' OR v.enviado_impuestos IS NULL)";
    ...
    public function Datos5()
    {
        ...
        {
            $ifecha = date('Y-m-d',strtotime($_POST['ifecha']));
            $stm = $this->db->prepare("SELECT fec_ven,desc_td,ser_doc,nro_doc,(total-desc_monto) AS total FROM v_ventas_con WHERE id_tdoc = 1 AND (enviado_impuestos IS NULL OR enviado_impuestos = '') AND DATE_FORMAT(fec_ven,'%Y-%m-%d') = ?");
    ...
    public function Detalle()
    {
        ...
        {      
            ...
            foreach($c as $k => $d)
            {
                $c[$k]->{'Cliente'} = $this->db->query("SELECT ci,nombre FROM v_clientes WHERE id_cliente = ".$d->id_cli)
                    ->fetch(PDO::FETCH_OBJ);
            }
            $data = array("data" => $c);
            $json = json_encode($data);
    ...
    {
        try {
            $stm = $this->db->prepare("UPDATE tm_venta SET enviado_impuestos = 1, code_respuesta_impuestos = 0 WHERE id_venta = ?");
